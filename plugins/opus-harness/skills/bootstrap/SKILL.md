...
1. **Protocol → CLAUDE.md**
   - Read `${CLAUDE_PLUGIN_ROOT}/skills/bootstrap/assets/protocol.md` — the
     tool-agnostic principles — and
     `${CLAUDE_PLUGIN_ROOT}/skills/bootstrap/assets/bindings.md` — how THIS
     environment implements them. (In the repo the protocol asset is a symlink
     to the canonical root `protocol.md`; plugin install dereferences it, so
     the installed copy is a real file. The protocol never references the
     plugin — keep it that way when editing.)
   - No CLAUDE.md in the project → write: the protocol, with this provenance
     line under the title, then the bindings section appended at the end:
   ...
   - CLAUDE.md exists → do NOT overwrite. Show the user a proposed merge
     (their content preserved, protocol + bindings appended or interleaved)
